...
export function enhancedProvideCompletionItems(
  ...
  completions.items = completions.items.filter(isValidCompletion).map(
    (completion) => {
      const source = completion?.data?.originalItem?.source;
      if (source) {
        // Sort completions starting with `civet:` higher than other imports
        if (source.startsWith("civet:")) {
          completion.sortText = "\u0000" +
...
export function enhancedResolveCompletionItem(
  ...
  if (resolvedCompletion.additionalTextEdits) {
    ...
    if (!virtualFile || !(code instanceof CivetVirtualCode)) {
      return resolvedCompletion;
    }

    resolvedCompletion.additionalTextEdits = resolvedCompletion
...
function isValidCompletion(completion: CompletionItem) {
  const isSvelte2tsxCompletion = completion.label.startsWith("__sveltets_") ||
    svelte2tsxTypes.has(completion.label);

  // Filter out completions for the children prop, as it doesn't work in Civet
