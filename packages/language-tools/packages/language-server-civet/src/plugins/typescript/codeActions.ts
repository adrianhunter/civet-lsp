...
export function enhancedResolveCodeAction(
...
function mapCodeAction(codeAction: CodeAction, context: ServiceContext) {
  ...
  codeAction.edit.documentChanges = codeAction.edit.documentChanges.map(
    (change) => {
      if (TextDocumentEdit.is(change)) {
        ...
        if (!virtualFile || !(code instanceof CivetVirtualCode)) return change;
