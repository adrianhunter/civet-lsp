...
export function enhancedProvideSemanticDiagnostics(
  ...
  const diagnostics = originalDiagnostics
    ...
    .map((diag) =>
      tsxLineCount
        ? generalEnhancements(civetEnhancements(diag))
        : generalEnhancements(diag)
    );
...
function generalEnhancements(diagnostic: Diagnostic) {
  if (
    ...
    diagnostic.message.includes("civet:content")
  ) {
    diagnostic.message +=
      "\n\nIf you're using content collections, make sure to run `civet dev`, `civet build` or `civet sync` to first generate the types so you can import from them. If you already ran one of those commands, restarting the language server might be necessary in order for the change to take effect.";
...
/**
 * Civet-specific enhancements. For instance, when the user tries to import a component from a framework that is not installed
 * or a difference with JSX needing a different error message
 */
function civetEnhancements(diagnostic: Diagnostic): Diagnostic {
  ...
  if (diagnostic.code === DiagnosticCodes.IS_NOT_A_MODULE) {
    if (diagnostic.message.includes(".svelte")) {
      diagnostic.message +=
        "\n\nIs the `@civetjs/svelte` package installed? You can add it to your project by running the following command: `civet add svelte`. If already installed, restarting the language server might be necessary in order for the change to take effect.";
    }

    if (diagnostic.message.includes(".vue")) {
      diagnostic.message +=
        "\n\nIs the `@civetjs/vue` package installed? You can add it to your project by running the following command: `civet add vue`. If already installed, restarting the language server might be necessary in order for the change to take effect.";
    }
...
/**
 * Civet allows multiple attributes to have the same name
 */
