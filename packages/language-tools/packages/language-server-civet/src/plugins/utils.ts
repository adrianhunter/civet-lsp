 ...
 */
export function isInComponentStartTag(
  html: HTMLDocument,
  offset: number,
): boolean {
 ...
 */
export function isInsideExpression(
  html: string,
  tagStart: number,
  position: number,
) {
...
export function ensureProperEditForFrontmatter(
  ...
  switch (metadata.frontmatter.status) {
    ...
    case "closed":
      const newRange = ensureRangeIsInFrontmatter(
        edit.range,
        metadata,
        position,
      );
...
export function ensureRangeIsInFrontmatter(
  ...
  if (
    metadata.frontmatter.status === "open" ||
...
export function getNewFrontmatterEdit(
  edit: TextEdit,
  civetMetadata: CivetMetadata,
  newLine: string,
) {
  edit.newText = `---${
    edit.newText.startsWith(newLine) ? "" : newLine
  }${edit.newText}---${newLine}${newLine}`;
  edit.range = Range.create(
    civetMetadata.tsxRanges.frontmatter.start,
    civetMetadata.tsxRanges.frontmatter.start,
  );
...
export function editShouldBeInFrontmatter(
  ...
  const isAtTSXStart =
    range.start.line < civetMetadata.tsxRanges.frontmatter.start.line;
