...
function isInComponentStartTag(html, offset) {
    ...
    return isPossibleComponent(node) &&
    ...
    switch (frontmatter.status) {
        case "closed":
            return offset > frontmatter.position.start.offset &&
                offset < frontmatter.position.end.offset;
        case "open":
            return offset > frontmatter.position.start.offset;
        case "doesnt-exist":
            return false;
...
function ensureRangeIsInFrontmatter(range, metadata, position = "top") {
    if (metadata.frontmatter.status === "open" ||
        metadata.frontmatter.status === "closed") {
...
function getNewFrontmatterEdit(edit, civetMetadata, newLine) {
    edit.newText = `---${edit.newText.startsWith(newLine) ? "" : newLine}${edit.newText}---${newLine}${newLine}`;
    edit.range = vscode_html_languageservice_1.Range.create(civetMetadata.tsxRanges.frontmatter.start, civetMetadata.tsxRanges.frontmatter.start);
    return edit;
}
exports.getNewFrontmatterEdit = getNewFrontmatterEdit;
function getOpenFrontmatterEdit(edit, civetMetadata, newLine) {
...
function editShouldBeInFrontmatter(range, civetMetadata) {
    const isAtTSXStart = range.start.line < civetMetadata.tsxRanges.frontmatter.start.line;
    const isPastFile = range.start.line > civetMetadata.tsxRanges.body.end.line;
