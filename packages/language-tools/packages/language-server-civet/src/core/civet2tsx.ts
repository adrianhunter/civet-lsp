      ...
      diagnostics: [
        {
          ...
          severity: 1,
          text:
            `The Civet compiler encountered an unknown error while transform this file to TSX. Please create an issue with your code and the error shown in the server's logs: https://github.com/withcivet/language-tools/issues`,
        },
      ],
      metaRanges: {
        frontmatter: {
      ...
      if (current) {
        ...
        if (length > 0) {
          ...
          if (
            ...
            const rangeCapabilities: CodeInformation = node.tag !== "script"
              ? {
                verification: true,
                completion: true,
                semantic: true,
