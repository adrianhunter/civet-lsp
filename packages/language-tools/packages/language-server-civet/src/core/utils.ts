...
export function framework2tsx(
  ...
  const integrationEditorEntrypoint = framework === "vue"
    ? importVueIntegration(filePath)
    : importSvelteIntegration(filePath);
  ...
  const tsx = patchTSX(
    integrationEditorEntrypoint.toTSX(sourceCode, className),
    filePath,
  );
...
export function classNameFromFilename(filename: string): string {
  ...
  const withoutExtensions = Utils.basename(url).slice(
    0,
    -Utils.extname(url).length,
  );
  ...
  const withoutLeadingInvalidCharacters = withoutInvalidCharacters.substring(
    firstValidCharIdx,
  );
  const inPascalCase = toPascalCase(withoutLeadingInvalidCharacters);
  const finalName = firstValidCharIdx === -1
    ? `A${inPascalCase}`
    : inPascalCase;
