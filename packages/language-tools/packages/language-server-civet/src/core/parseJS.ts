...
export function extractScriptTags(
  ...
  const embeddedJSCodes: VirtualCode[] = findModuleScripts(
    snapshot,
    htmlDocument.roots,
  );
...
function getScriptType(
  scriptTag: Node,
): "classic" | "module" | "processed module" {
  ...
  function getEmbeddedScriptsInNodes(nodes: Node[]) {
    for (const [_, node] of nodes.entries()) {
      if (
        ...
        const extension = getScriptType(node) === "processed module"
          ? "mts"
          : "mjs";
        const languageId = getScriptType(node) === "processed module"
          ? "typescript"
          : "javascript";
    ...
    parent.children.forEach((child) => {
      if (is.element(child)) {
        const eventAttribute = child.attributes.find(
          (attr) =>
            htmlEventAttributes.includes(attr.name) && attr.kind === "quoted",
        );
        ...
        if (eventAttribute && eventAttribute.position) {
          eventAttrs.push({
            ...
            startOffset: eventAttribute.position.start.offset +
