...
export type CivetMetadata = ParseResult & {
  frontmatter: FrontmatterStatus;
  tsxRanges: LSPTSXRanges;
};

export function getCivetMetadata(
  fileName: string,
  input: string,
  options: ParseOptions = { position: true },
): Omit<CivetMetadata, "tsxRanges"> {
...
function safeParseAst(
  ...
  try {
    ...
    console.error(
      `There was an error parsing ${fileName}'s AST. An empty AST will be returned instead to avoid breaking the server. Please create an issue: https://github.com/withcivet/language-tools/issues\nError: ${e}.`,
    );
      ...
      diagnostics: [
        {
          ...
          severity: 1,
          text:
            `The Civet compiler encountered an unknown error while parsing this file's AST. Please create an issue with your code and the error shown in the server's logs: https://github.com/withcivet/language-tools/issues`,
        },
      ],
    };
  }
  ...
  if (ast.children[0].type === "frontmatter") {
    ...
    if (frontmatter.position) {
      if (frontmatter.position.end) {
        ...
        if (
          frontmatter.position.end.offset === text.length &&
          !text.endsWith("---")
        ) {
          return {
            status: "open",
