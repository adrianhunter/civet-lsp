...
export class CivetCheck {
  ...
  public async lint({
    ...
    let files = (
      fileNames !== undefined
        ? fileNames
        : this.linter.languageHost.getScriptFileNames()
    ).filter((file) => {
  ...
  private initialize() {
    this.ts = this.typescriptPath
      ? require(this.typescriptPath)
      : require("typescript");
    const tsconfigPath = this.getTsconfig();

    const civetInstall = getCivetInstall([this.workspacePath]);
    const languages = [
      getLanguageModule(
    ...
    const services = [
      ...createTypeScriptServices(this.ts),
      createCivetService(this.ts),
    ];
  ...
  private getTsconfig() {
    if (this.tsconfigPath) {
      const tsconfig = resolve(
        this.workspacePath,
        this.tsconfigPath.replace(/^~/, homedir()),
      );
