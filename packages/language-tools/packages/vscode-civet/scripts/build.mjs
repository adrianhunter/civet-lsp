  ...
  const config = {
    ...
    plugins: [
      copy({
        resolveFrom: "cwd",
        assets: {
          from: ["../language-server-civet/types/**/*.d.ts"],
          to: ["./dist/types"],
          watch: isDev,
        },
      }),
      {
        ...
        setup(pluginBuild) {
          pluginBuild.onResolve(
            ...
            (buildArgs) => {
              const pathUmdMay = require.resolve(buildArgs.path, {
                paths: [buildArgs.resolveDir],
              });
              // Call twice the replace is to solve the problem of the path in Windows
              const pathEsm = pathUmdMay.replace("/umd/", "/esm/").replace(
                "\\umd\\",
                "\\esm\\",
              );
              return { path: pathEsm };
