import { serialize } from "@civetjs/compiler";
import {
...
export function isInlineElement(
  ...
  return node && isTagLikeNode(node) && !isBlockElement(node, opts) &&
...
export function isNodeWithChildren(
  node: anyNode,
): node is anyNode & ParentLikeNode {
...
export function getNextNode(path: AstPath): anyNode | null {
  ...
  if (node) {
    ...
    if (
      node.position?.start === siblings[siblings.length - 1].position?.start
    ) return null;
...
export function getPreferredQuote(
  ...
  if (
    rawContent.includes(preferred.quote) || rawContent.includes(alternate.quote)
  ) {
...
export function inferParserByTypeAttribute(type: string): BuiltInParserName {
  ...
  switch (type) {
    ...
    default:
      if (
        type.endsWith("json") || type.endsWith("importmap") ||
