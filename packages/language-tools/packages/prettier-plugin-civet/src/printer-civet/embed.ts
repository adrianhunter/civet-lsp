...
export const embed = ((path: AstPath, options: Options) => {
  ...
  return async (textToDoc, print) => {
    ...
    console.log("🚗 --------------------------------------------🚗");
    console.log("🚗 ~ file: embed.ts:70 ~ return ~ node:", node);
    console.log("🚗 --------------------------------------------🚗");
    ...
    if (node.type === "expression") {
      ...
      content = await wrapParserTryCatch(textToDoc, textContent, {
        ...options,
        parser: "civetExpressionParser",
      });

      content = stripTrailingHardline(content);

      console.log("🚗 --------------------------------------------------🚗");
      console.log("🚗 ~ file: embed.ts:90 ~ return ~ content:", content);
      console.log("🚗 --------------------------------------------------🚗");
    ...
    if (node.type === "frontmatter") {
      const frontmatterContent = await wrapParserTryCatch(
        textToDoc,
        node.value,
        {
          ...options,
      ...
      return [
        group(["---", hardline, frontmatterContent, hardline, "---", hardline]),
        hardline,
      ];
    }
    ...
    // Script tags
    if (
      node.type === "element" && node.name === "script" && node.children.length
    ) {
      const typeAttribute = node.attributes.find((attr) => attr.name === "type")
        ?.value;
      ...
      if (node.attributes) {
        ...
        if (langAttribute.length) {
          const styleLang = langAttribute[0].value
            .toLowerCase() as supportedStyleLang;
          parserLang = supportedStyleLangValues.includes(styleLang)
            ? styleLang
            : undefined;
        }
      }

      return await embedStyle(
        parserLang,
        content,
        path,
        print,
...
function makeNodeJSXCompatible<T>(node: any): T {
  ...
  if (isNodeWithChildren(newNode)) {
    newNode.children = newNode.children.reduce(
...
async function embedStyle(
  ...
  switch (lang) {
    ...
    case "sass": {
      const lineEnding = options?.endOfLine?.toUpperCase() === "CRLF"
        ? "CRLF"
        : "LF";
      ...
      // format
      const formattedSassIndented = SassFormatter.Format(raw, sassOptions)
        .trim();
