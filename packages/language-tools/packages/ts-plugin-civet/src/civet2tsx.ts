...
function safeConvertToTSX(content: string, options: ConvertToTSXOptions) {
  try {
    ...
    console.error(
      `There was an error transforming ${options.filename} to TSX. An empty file will be returned instead. Please create an issue: https://github.com/withcivet/language-tools/issues\nError: ${e}.`,
    );
      ...
      diagnostics: [
        {
          ...
          severity: 1,
          text:
            `The Civet compiler encountered an unknown error while parsing this file. Please create an issue with your code and the error shown in the server's logs: https://github.com/withcivet/language-tools/issues`,
        },
      ],
      metaRanges: {
        frontmatter: {
...
export function civet2tsx(input: string, fileName: string) {
...
function getVirtualFileTSX(
  ...
  const sourcedDoc = TextDocument.create(fileName, "civet", 0, input);
  const genDoc = TextDocument.create(
    fileName + ".tsx",
    "typescriptreact",
    0,
    tsx.code,
  ...
  for (let genLine = 0; genLine < v3Mappings.length; genLine++) {
    for (const segment of v3Mappings[genLine]) {
      ...
      const genOffset = genDoc.offsetAt({
        line: genLine,
        character: genCharacter,
      });
      if (current) {
        let length = genOffset - current.genOffset;
        const sourceText = input.substring(
      ...
      if (segment[2] !== undefined && segment[3] !== undefined) {
        const sourceOffset = sourcedDoc.offsetAt({
          line: segment[2],
          character: segment[3],
        });
        current = {
...
function patchTSX(code: string, fileName: string) {
  ...
  return code;
  // return code.replace(/\b(\S*)__CivetComponent_/gm, (fullMatch, m1: string) => {
